...
MemberWidget::MemberWidget(const QJsonObject& data, QWidget* parent) : QFrame(parent) {
    ...
    nameLabel->setStyleSheet("color: #FFFFFF; background: transparent; font-size: 14px; font-weight: bold;");
    layout->addWidget(nameLabel);

    layout->addStretch();

    micIcon = new QSvgWidget(this);
    micIcon->setFixedSize(20, 20);
    micIcon->setStyleSheet("background: transparent;");
    layout->addWidget(micIcon);

    deafenIcon = new QSvgWidget(this);
    deafenIcon->setFixedSize(20, 20);
    deafenIcon->setStyleSheet("background: transparent;");
...
void MemberWidget::updateData(const QJsonObject& data) {
    ...
    if (isFirstUpdate) {
        ...
        nameLabel->setStyleSheet(QString("color: rgba(255, 255, 255, %1); background: transparent; font-size: 14px; font-weight: bold;")
                                 .arg((muted || deafened) ? "0.6" : "1.0"));
    } else {
...
void MemberWidget::animateStateChange(bool muted, bool deafened) {
    ...
    connect(colorAnim, &QVariantAnimation::valueChanged, this, [this](const QVariant &val){
        nameLabel->setStyleSheet(QString("color: rgba(255, 255, 255, %1); background: transparent; font-size: 14px; font-weight: bold;").arg(val.toDouble()));
    });
    colorAnim->start(QAbstractAnimation::DeleteWhenStopped);
}

void MemberWidget::updateStatusIcons(bool muted, bool deafened) {
    bool effectiveMute = muted || deafened;
    micIcon->load(effectiveMute ? SVG_MIC_OFF : SVG_MIC_ON);
    deafenIcon->load(deafened ? SVG_HEADSET_OFF : SVG_HEADSET_ON);
}
